...
async function install(fs, gyp, argv) {
	...
	if (semver.lt(release.version, "0.8.0")) {
		throw new Error(
			"Minimum target version is `0.8.0` or greater. Got: " +
				release.version,
		);
	}
	...
	async function go() {
		...
		async function downloadShasums() {
			...
			if (res.status !== 200) {
				throw new Error(
					`${res.status}  status code downloading checksum`,
				);
			}
		...
		function downloadNodeLib() {
			...
			return archs.map(async (arch) => {
				...
				if (res.status === 403 || res.status === 404) {
					...
					throw new Error(
						`${res.status} status code downloading ${name}`,
					);
				}
