			...
			var checks = [
				...
				},
				{
					before: () => {
						...
						this.addLog(
							`- process.env.PYTHON is "${this.env.PYTHON}"`,
						);
					},
					check: this.checkCommand,
					arg: this.env.PYTHON,
			...
			if (this.win) {
				for (var i = 0; i < this.winDefaultLocations.length; ++i) {
					...
					checks.push({
						before: () => {
							this.addLog(
								"checking if Python is " + `${location}`,
							);
						},
	...
	checkCommand: function checkCommand(command, errorCallback) {
		...
		this.run(
			...
			function (err, execPath) {
				...
				if (err) {
					this.addLog(
						`- "${command}" is not in PATH or produced an error`,
					);
		...
		this.run(
			...
			function (err, version) {
				...
				try {
					...
					this.addLog(
						`- "${execPath}" does not have a valid version`,
					);
